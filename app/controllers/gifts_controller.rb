=begin class GiftsController < Controller
    ...
    def set_gift_price(message)
        user = @db.find_or_create_user_by_chat_id(message.from.id)
        @db.update_gift_price(price: message.text, id: user[:current_wishlist_id])
        @db.update_user(state: 'initial_state', chat_id: message.from.id)
        @bot.api.send_message(chat_id: message.from.id, text: "Подарое добавлен!")
    end    
end =end